# @title 📂 Gram Panchayat PDF Processor (Fixed for Side-by-Side Layout)
# @markdown ### Instructions:
# @markdown 1. Run this cell.
# @markdown 2. Upload your 5 (or more) PDFs.
# @markdown 3. The script will split each page vertically to correctly separate Receipts and Payments.
...
# --- 2. PARSING LOGIC (Applied to a block of text) ---
def parse_text_block(text_lines, dist, block, gp, section_type):
    ...
    for line in text_lines:
        ...
        # Skip headers
        if "RECEIPTS" in line or "PAYMENTS" in line or "HEADS OF ACCOUNT" in line or "Budget" in line:
            continue

        # --- Regex Logic ---
        
        # 1. Major Head (4 digits)
        # matches "1601 Grants-in-aid" or "2515 Panchayati..."
        ...
        # Looks for: (2-3 chars code) (Text description) (Amount)
        # Handles amounts like "1,000.00" or "500"
        # We look for the numeric amount at the end of the line
        ...
        if match_obj:
            ...
            # Only add if amount > 0 or logic requires it
            if amount >= 0:
                data_rows.append({
                    "Dist_Name": dist,
                    "Block_Name": block,
...
# --- 3. PDF PROCESSING ENGINE (Split Page) ---
...
def main():
    ...
    print("\nStarting Processing (Split-Page Mode)...")
    ...
    for filename, content in uploaded.items():
        ...
        try:
            ...
            print(f"✅ Processed {filename}: Found {len(r_data)} Receipts, {len(p_data)} Payments")
